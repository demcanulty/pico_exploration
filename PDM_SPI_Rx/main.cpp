...
s16 * audio_buff = buffer_1;
bool halt_audio_out=true;
    ...
    while (1) 
    {
        ...
        if(!halt_audio_out)
        {
            for (int i = 0; i < 2; i++) 
            {
                ...
                {   
                    generate_pdm_words(ch, *audio_buff++);
...
void core1_main()
{
    ...
    while(true)
    {
        ...
        switch(this_buff)
        {
            case 0:
            {
                start_dma_receive((uint8_t *)buffer_1, TRANSFER_SIZE);
                dma_channel_wait_for_finish_blocking(spi_dma_chan);

                this_buff = 1;
                halt_audio_out = true;
                audio_buff = buffer_1;
                halt_audio_out = false;
            }
            break;
            ...
            case 1:
            {
                ...
                start_dma_receive((uint8_t *)buffer_2, TRANSFER_SIZE);
                dma_channel_wait_for_finish_blocking(spi_dma_chan);
